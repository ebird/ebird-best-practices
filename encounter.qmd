...
To ensure bias in the test data set doesn't impact the predictive performance metrics, it's important that we apply spatiotemporal grid sampling to the test data just as we did to the training data. We already performed this [grid sampling above](#sec-encounter-sss) when we created the `checklists_ss` data frame, so we use that data frame here to calculate the PPMs.
...
em <- precrec::evalmod(scores = obs_pred_test$pred_calibrated,
...
-   `calibration_model`: the calibration model object
