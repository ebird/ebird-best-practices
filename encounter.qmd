...
## Introduction {#sec-encouter-intro}
...
Let's grab the top 15 most important predictors, which we'll need in the next section.

```{r}
#| label: encounter-habitat-pi-top
# top predictors other than date
top_pred <- pi %>% 
...
Now we'll use this function to calculate partial dependence for the top 15 predictors.
...
ggplot(pd) +
  ...
  geom_line() +
  geom_point() +
  facet_wrap(~ as_factor(predictor), ncol = 3, scales = "free") +
...
Now for the fun part: let's use the calibrated random forest model to make a map of Wood Thrush encounter rate in Georgia! In @sec-envvar-pred, we created a prediction grid consisting of the habitat variables summarized on a regular grid of points across the study region. In this section, we'll make predictions of encounter rate at these points.

### Standardized effort variables {#sec-encounter-predict-effort}

The prediction grid only includes values for the environmental variables, so to make predictions we'll need to add effort variables to this prediction grid. We'll make predictions for a **standard eBird checklist**: a 1 km, 2 hour traveling count at the peak time of day for detecting this species. Finally, we'll make these predictions for June 15, 2023, the middle of our June focal window for the latest year for which we have eBird data.
...
pred_er_cal <- predict(calibration_model, 
                       ...
                       type = "response") %>% 
  as.numeric()
# add encounter rate estimate to prediction grid
predictions <- bind_cols(pred_grid_eff,
                         in_range = pred_binary, 
                         encounter_rate = pred_er_cal) %>% 
  select(cell_id, x, y, in_range, encounter_rate) %>% 
...
r_pred <- predictions %>% 
  ...
  rasterize(r, field = c("in_range", "encounter_rate"),
            fun = "mean") %>% 
  setNames(c("in_range", "encounter_rate"))

# save the raster
r_pred <- writeRaster(r_pred, "results/encounter-rate_woothr_june-2023.tif", 
                      overwrite = TRUE,
                      gdal = "COMPRESS=DEFLATE")
```

### Mapping {#sec-encounter-predict-map}
...
Although this encounter rate map provides more detailed information on the distribution of Wood Thrush, for many applications a simple range map may be more desirable. For this, we can map the `in_range` layer of the raster of predictions we created.
