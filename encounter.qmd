...
### Thresholding {#encoumter-rf-threshold}
...
To assess the quality of the calibrated random forest model, we'll validate the model’s ability to predict the observed patterns of detection using independent validation data (i.e. the 20% test data set). We'll use a range of predictive performance metrics (PPMs) to compare the predictions to the actual observations. [Mean squared error (MSE)](https://en.wikipedia.org/wiki/Mean_squared_error) and [Spearman's rank correlation coefficient](https://en.wikipedia.org/wiki/Spearman%27s_rank_correlation_coefficient) apply to the calibrated encounter rate estimates. The remaining metrics apply to the binary detection/non-detection predictions: [sensitivity](https://en.wikipedia.org/wiki/Sensitivity_and_specificity), [specificity](https://en.wikipedia.org/wiki/Sensitivity_and_specificity), [Precision-Recall AUC](https://machinelearningmastery.com/roc-curves-and-precision-recall-curves-for-imbalanced-classification/), [Kappa](https://en.wikipedia.org/wiki/Cohen%27s_kappa), [F1 score](https://en.wikipedia.org/wiki/F-score), and [MCC](https://en.wikipedia.org/wiki/Phi_coefficient).
...
```{r}
#| label:  encounter-predict-predict
...
```{r}
#| label:  encounter-predict-rasterize
r_pred <- predictions %>% 
