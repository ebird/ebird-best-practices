...
The relative abundance model presented here is similar to and a natural extension of the encounter rate model of Chapter \@ref(encounter). In particular, we use a two-step hurdle model following Keyser et al. [-@keyserSnowCoverDynamics2023]. In the first step, we estimate encounter rate using exactly the same method as in Chapter \@ref(encounter). In the second step, we estimate the expected count of individuals on eBird checklists where the species was detected. Finally, we multiple encounter rate by median count to produce an estimate of relative abundance. We use random forests for both steps on the hurdle.

## Data preparation {#encounter-data}

Let's get started by loading the necessary packages and data. If you worked through the previous chapters, you should have all the data required for this chapter. However, you may want to [download the data package](https://github.com/ebird/ebird-best-practices/raw/master/data/data.zip), and unzip it to your project directory, to ensure you're working with exactly the same data as was used in the creation of this book.
...
## Hurdle model {#abundance-hurdle}

For this two-step hurdle model, we'll start by training exactly the same encounter rate model as in the [previous chapter](#encounter). Then we'll subset the eBird checklist to only those where the species was detected or predicted to occur by the encounter rate model. We'll use this subset of the data to train a second random forest model for expected count. Finally we'll combine the results of the two steps together to produce estimates of relative abundance.

### Step 1: Encounter rate {#abundance-hurdle-er}
...
### Step 2: Expected count {#abundance-hurdle-count-}
...
Just as we did in the [previous chapter](#encounter-predict) for encounter rate, we can estimate relative abundance over our prediction surface. First we estimate encounter rate and count, then we multiply these together to get an estimate of relative abundance. Let's start by added the effort variables to the prediction grid for a standard eBird checklist at the optimal time of day for detecting Wood Thrush. Recall from the [previous chapter](#encounter-predict) that we determined the optimal time of day for detecting Wood Thrush was around 6:15AM.

```{r}
#| label: abundance-predict-surface
