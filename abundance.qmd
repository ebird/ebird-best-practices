...
Prior to mapping the relative abundance, we'll multiply by the `in_range` layer, which will produce a map showing zero relative abundance where the model predicts that Wood Thrush does not occur.
...
abd_in_range <- r_pred[["abundance"]] * r_pred[["in_range"]]
brks <- ifel(abd_in_range > 0, abd_in_range, NA) |> 
  global(fun = quantile, 
