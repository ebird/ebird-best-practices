...
The relative abundance model presented here is similar to and a natural extension of the encounter rate model of Chapter \@ref(encounter). In particular, we use a two-step hurdle model following Fink et al. [`@keyserSnowCoverDynamics2023]. In the first step, we estimate encounter rate using exactly the same method as in Chapter \@ref(encounter). In the second step, we estimate the expected count of individuals on eBird checklists where the species was detected. Finally, we multiple encounter rate by median count to produce an estimate of relative abundance. We use random forests for both steps on the hurdle.
