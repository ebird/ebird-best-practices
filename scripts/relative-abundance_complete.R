...
zf <- read_csv("data/checklists-zf_woothr_jun_us-ga.csv")
# combine ebird and environmental data
checklists <- inner_join(zf, env_vars, by = "checklist_id")
...
pi_er <- enframe(er_model$variable.importance, "predictor", "importance") %>%
  arrange(desc(importance))
pi_count <- enframe(count_model$variable.importance,
                    "predictor", "importance") %>%
  arrange(desc(importance))
...
# EXERCISE: Produce a the partial dependence plot for the checklist start time.
